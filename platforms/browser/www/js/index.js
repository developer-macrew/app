...
function sa() {
	...
	this.app_url = 'http://www.macrew.info/shevet/htdocs/api.php';//http://app.shevetahim.com/api.php';
...
sa.prototype.login = function (button, info) {
	...
	alert('login'+self.session.status);//macrew
	self.session.status = 'approved';
		self.loadSettings();
	alert('login'+result.User.login.results[0].has_logged_in);
...
sa.prototype.loadSettings = function (callback) {
	...
	this.sendRequests(function (result) {
		alert('loadSettings'+JSON.stringify(result));
		...
		alert('initial_status'+JSON.stringify(initial_status));
...
function initOneSignal() {
	var one_signal_app_id = 'c2befe0f-0c59-4577-a93f-ec80e24a3232';//test app Id

	window.plugins.OneSignal.startInit(one_signal_app_id).handleNotificationOpened(pushOpened).endInit();
