...
class User {
	...
	public static function login($info) {
		...
		$iv = bin2hex($nonce);
		 $q =  "SELECT site_users.* FROM site_users WHERE LOWER(site_users.email) = '".strtolower($info['email'])."'";
		...
		if (!$pass1) {
		 ...
		 $q =  "SELECT site_users.* FROM site_users WHERE LOWER(site_users.email) = '".strtolower($email1)."'";
		$result = db_query_array($q);
		if (!$result) {
			/*
